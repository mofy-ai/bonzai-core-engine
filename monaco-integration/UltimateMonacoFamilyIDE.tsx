      ...
      <div className="p-3 border-t border-purple-500/20">
        <form onSubmit={handleSubmit} className="space-y-2">
          <div className="flex items-center space-x-2">
            <div className="flex-1 relative">
              ...
              <Button
                ...
                variant="ghost"
                className={cn(
                  "absolute right-2 top-2 w-6 h-6 p-0",
                  isVoiceActive ? "text-red-400" : "text-purple-400"
                )}
                onClick={() => setIsVoiceActive(!isVoiceActive)}
              >
                {isVoiceActive ? <Volume2 className="w-3 h-3" /> : <Mic className="w-3 h-3" />}
              </Button>
            </div>
            <Button
              type="submit"
          ...
          <div className="flex items-center space-x-2 text-xs text-purple-400">
            <span>💜 @ mention files, use voice input, or just chat naturally!</span>
          </div>
        </form>
      </div>
    </div>
...
export const UltimateMonacoFamilyIDE: React.FC = () => {
